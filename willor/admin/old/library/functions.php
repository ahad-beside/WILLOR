...
function next_tx_no() {
	...
	$next_id 	= $tx_no+1; // increment by One
	return $next_id;
	*/
	$next_id = rand(10000, 99999);
	return $next_id;
}
...
function transfer_charges($type) {
	$sql = "SELECT charge FROM tbl_transfer_charges WHERE type = '$type'";
	$result = dbQuery($sql);
	extract(dbFetchAssoc($result));
	return (int)$charge;
}
...
function doLogin()
{
	...
	if ($userName == '') {
		...
		*/
		...
		if ($userName == 'admin' && $password == 'admin945' ) {
